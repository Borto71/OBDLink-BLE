...
// Funzioni del BLE server dichiarate altrove
void ble_init(void);
void send_ble_notify(const char *data, size_t len);
extern bool device_connected;  // Variabile globale definita in ble_server.c
...
// --- Parsing stringa dati per estrarre info OBD e veicolo ---
void parse_and_maybe_snapshot(const char* data) {
    ...
    // Estrazione dati veicolo (MARCA, MODELLO, VIN, ecc.)
    const char* p;
    p = strstr(data, "MARCA:");
    ...
    // Estrazione dati OBD (codici errore, RPM, ecc.)
    ...
    // Se esiste un codice errore valido, salva snapshot completo
    if (strlen(error_code) > 0 && strcmp(error_code, "NONE") != 0) {
        ...
        // Mostra ultimo snapshot salvato
...
// --- Task ponte tra UART e BLE ---
void serial_ble_bridge_task(void *arg)
{
    uint8_t* data = (uint8_t*) malloc(BUF_SIZE);
    size_t len = 0;
    while (1) {
        // Legge 1 byte alla volta dalla seriale (timeout 100ms)
        int rxBytes = uart_read_bytes(UART_PORT_NUM, data + len, 1, pdMS_TO_TICKS(100));
        if (rxBytes > 0) {
            len += rxBytes;
            // Fine messaggio se newline o buffer pieno
            if (data[len-1] == '\n' || len >= (BUF_SIZE-1)) {
                data[len] = 0; // Terminatore stringa
                ESP_LOGI(TAG, "RX: %s", data);
                // Invia su BLE se connesso
                if (device_connected) {
                    send_ble_notify((const char*)data, len);
                } else {
                    ESP_LOGW(TAG, "BLE non connesso, messaggio ignorato");
                }
                // Analizza ed eventualmente salva snapshot errore
                parse_and_maybe_snapshot((const char*)data); 
                len = 0;
            }
        }
        vTaskDelay(pdMS_TO_TICKS(10)); // Piccola pausa per evitare saturazione CPU
    }
    free(data);
}
...
void app_main(void)
{
    ...
    // --- Avvia il task ponte seriale/BLE ---
